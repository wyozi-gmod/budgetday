-- Entity directly stolen from base gmod (where it's known as "gmod_lamp")
-- We need to decrease lamp sprite brightness so the game is somewhat playable with nightvision, which is not doable
...
function ENT:DrawTranslucent()
	...
	render.StartBeam( 2 )
		render.AddBeam( LightPos - LightNrm * 1, 64, 0.0, Color( r, g, b, 80 * BeamDot) )
	...
	if ( ViewDot >= 0 ) then
		...
		local Size = math.Clamp( Distance * Visibile * ViewDot * 0.3, 64, 512 )
		
		Distance = math.Clamp( Distance, 32, 800 )
		local Alpha = math.Clamp( (1000 - Distance) * Visibile * ViewDot * 0.5, 0, 100 )
